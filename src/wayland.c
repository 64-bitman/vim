...
typedef struct {
    ...
    uint32_t wl_data_device_manager_name;

    // Focus stealing method
...
// Struct wrapper for an xdg surface
typedef struct {
...
typedef struct {
    ...
    // The transparent surface that will be displayed
    vwl_xdg_surface *shell_surface;
} vwl_clipboard_T;
...
static struct wl_keyboard * vwl_seat_init_keyboard(vwl_seat_T *seat);

#ifdef FEAT_WAYLAND_CLIPBOARD

static void vwl_cb_buffer_listener_release( void *data UNUSED,
...
#define rm_gobject(object) \
    ...
    rm_gobject(wl_data_device_manager)
    rm_gobject(wl_shm)
    rm_gobject(wl_compositor)
    rm_gobject(xdg_wm_base)
...
{
    if (seat->keyboard.proxy != NULL)
    {
	wl_keyboard_destroy(seat->keyboard.proxy);
	seat->keyboard.proxy = NULL;
    }
    if (seat->proxy != NULL)
    {
...
/*
 * Initialize keyboard object for seat and return it. NULL is returned on
 * failure such as when a keyboard is not available for seat.
 */
    static struct wl_keyboard *
vwl_seat_init_keyboard(vwl_seat_T *seat)
{
    if (!(seat->capabilities & WL_SEAT_CAPABILITY_KEYBOARD))
	return NULL;

    seat->keyboard.proxy = wl_seat_get_keyboard(seat->proxy);
    return seat->keyboard.proxy;
}
...
#define set_manager(manager_name, protocol_enum, focus) \
    ...
    if (vwl_gobjects.wl_compositor != NULL && vwl_gobjects.wl_shm != NULL)
    {
...
wayland_cb_init(const char *seat)
{
    ...
    if (p_wst && (vwl_clipboard.regular.requires_focus
	...
	// If fd is -1, any attempts to use the data device manager should fail
	// since focus is never gained.
	if (r == 0)
	{
	    // Prepare buffer
	    ...
	    // Init keyboard object to be used later
	    vwl_seat_init_keyboard(vwl_clipboard.seat);
	}
    }
