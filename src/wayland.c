...
typedef struct wayland_cb_request_data_S wayland_cb_request_data_T;
typedef struct wayland_cb_send_data_S wayland_cb_send_data_T;
...
struct vwl_data_source_S {
    ...
    wayland_selection_T selection; // Selection that owns this source
};
...
// Struct to be passed around when requesting data
struct wayland_cb_request_data_S {
    wayland_cb_receive_data_func_T receive_data;
    wayland_cb_choose_offer_func_T choose_offer;
    void *user_data;
    int free_strings;
};

// Struct to be passed around when sending data;
struct wayland_cb_send_data_S {
    wayland_cb_send_data_func_T send_data;
    void *user_data;
};
...
static int vwl_data_device_manager_create_data_source(vwl_data_source_T *source,
	vwl_data_device_manager_T *manager, wayland_selection_T selection);
...
static void wayland_cb_data_source_send(vwl_data_source_T *source,
	const char *mime_type, int fd);
static void wayland_cb_data_source_cancelled(vwl_data_source_T *source);
...
wayland_cb_request_data_T wayland_cb_request_data;
wayland_cb_send_data_T wayland_cb_send_data;
#endif
...
vwl_log_handler(const char *fmt, va_list args)
{
    ...
    vim_snprintf(format, len - 1, "wayland protocol error: %s", fmt);
...
vwl_registry_get_data_device_manager(vwl_data_device_manager_T *manager,
	vwl_registry_T *registry, wayland_sbitmask_T selection)
{
    vim_memset(manager, 0, sizeof(*manager));
...
vwl_data_device_manager_create_data_source(vwl_data_source_T *source,
	vwl_data_device_manager_T *manager, wayland_selection_T selection)
{
    vim_memset(source, 0, sizeof(*source));
...
{
    vim_memset(offer, 0, sizeof(*offer));
	...
	vwl_data_source_T *source)
{
    ...
    {
	if (source->selection == WAYLAND_SELECTION_PRIMARY)
...
wayland_cb_init(const char *seat)
{
    ...
    // Use regular instead if primary not available
    if (ret == FAIL)
	ret = vwl_registry_get_data_device_manager(
		&vwl_clipboard.primary.manager, &vwl_display.registry,
		WAYLAND_SELECTION_REGULAR);

    ret = vwl_data_device_manager_get_data_device(
	    &vwl_clipboard.regular.source_device,
	    &vwl_clipboard.regular.manager, vseat,
	    WAYLAND_SELECTION_REGULAR);
    ret = vwl_data_device_manager_get_data_device(
			  ...
			  // an array of heap allocated strings.
{
	...
	manager = &vwl_clipboard.primary.manager;
    }
    else if (selection == WAYLAND_SELECTION_REGULAR)
    {
	manager = &vwl_clipboard.regular.manager;
    ...
    wayland_cb_request_data.receive_data = receive_callback;
    wayland_cb_request_data.choose_offer = offer_callback;
    wayland_cb_request_data.user_data = user_data;
    wayland_cb_request_data.free_strings = free_strings;

    device->data = &wayland_cb_request_data;
...
{
    wayland_cb_request_data_T *data = offer->data;
    wayland_cb_choose_offer_func_T callback = data->choose_offer;

    callback(data->user_data, mime_type, &offer->mime_types);
}

    static void
wayland_cb_data_device_selection(
	vwl_data_device_T *device,
	wayland_selection_T selection)
{
    wayland_cb_request_data_T *data = device->data;
    wayland_cb_receive_data_func_T callback = data->receive_data;
	...
	if (vwl_display_flush(&vwl_display) == OK)
	    callback(data->user_data, mime_type, fds[0], selection);
    }
...
wayland_cb_selection_is_owned(wayland_selection_T selection)
{
    ...
    if (selection == WAYLAND_SELECTION_PRIMARY)
	source = &vwl_clipboard.primary.source;
    else if (selection == WAYLAND_SELECTION_REGULAR)
	source = &vwl_clipboard.regular.source;
...
{
    if (selection == WAYLAND_SELECTION_PRIMARY)
	vwl_data_source_destroy(&vwl_clipboard.primary.source);
    else if (selection == WAYLAND_SELECTION_REGULAR)
	vwl_data_source_destroy(&vwl_clipboard.regular.source);
}
...
wayland_cb_own_selection(
	...
	int len,
	int force, // Destroy existing data source if it exists, useful if you
		   // want to use a different callback function?
	void *user_data)
{
    vwl_data_device_manager_T *manager;
    vwl_data_device_T *device;
    ...
    if (force)
	vwl_data_source_destroy(source);
    else
	if (source->proxy != NULL)
	    return OK;

    ret = vwl_data_device_manager_create_data_source(
	    source, manager, selection);

    if (ret == FAIL)
	return FAIL;

    wayland_cb_send_data.send_data = send_callback;
    wayland_cb_send_data.user_data = user_data;

    source->data = &wayland_cb_send_data;
    source->listener.send = wayland_cb_data_source_send;
    source->listener.cancelled = wayland_cb_data_source_cancelled;

    vwl_data_source_listen(source);

    for (int i = 0; i < len; i++)
	vwl_data_source_offer(source, mime_types[i]);

    vwl_data_device_set_selection(device, source);
    ...
    static void
wayland_cb_data_source_send(
	vwl_data_source_T *source,
	const char *mime_type,
	int fd)
{
    wayland_cb_send_data_T *data = source->data;
    wayland_cb_send_data_func_T callback = data->send_data;

    callback(data->user_data, mime_type, fd, source->selection);

    close(fd);
}

    static void
wayland_cb_data_source_cancelled(vwl_data_source_T *source)
{
    vwl_data_source_destroy(source);
}
...
wayland_cb_is_ready(void)
{
    ...
    if (!wayland_client_is_connected() || c->seat == NULL)
	return FALSE;
...
// TODO: generate wayland protocol files when needed instead
