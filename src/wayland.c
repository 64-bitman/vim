...
typedef struct
{
    char *mimetype; // current mime type
...
/*
 * Called immediately after data_offer event, choose UTF-8 as the encoding
 * to receive the data in (TODO: support other formats?).
 */
    static void
	...
	struct zwlr_data_control_offer_v1 *offer, const char *mime)
{
    ...
    // We already are processing an offer, skip this
    if (state->offer.zwlr != NULL)
	return;

    if (STRCMP(mime, mime_plain_utf8) == 0)
	state->mimetype = mime_plain_utf8;
    else
	return;
