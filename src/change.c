	...
	int	    *which_pat)
{
	...
	*which_pat = RE_SUBST;	// use last substitute regexp
    ...
    if (cmdname[0] == 's' && *cmd != NUL && !VIM_ISWHITE(*cmd)
	...
	if (check_regexp_delim(*cmd) == FAIL)
	    return FAIL;
#ifdef FEAT_EVAL
	if (in_vim9script() && check_global_and_subst(cmdname, arg) == FAIL)
	    return FAIL;
#endif
	...
	linenr_T    *last_line)
{
    ...
    // If top and bottom is -1, then go through each visible window, and find the
    // range of lines to perform substitution on. This is so we only perform
    // subsitution preview on lines that the user can actually see.
    if (line1 == -1 && line2 == -1)
    {
	win_T *wp;
	...
	FOR_ALL_WINDOWS_IN_TAB(curtab, wp)
	{
	    if (wp->w_buffer != curbuf)
		continue;
	    if (line1 == -1 || line1 > wp->w_topline)
		line1 = wp->w_topline;
	    if (line2 == -1 || line2 < wp->w_botline)
		line2 = wp->w_botline;
	}
    }
    if (line1 == -1 && line2 == -1)
	// Only can happen if buffer is not visible somehow?
	return;

    for (linenr_T lnum = line1; lnum <= line2 && !(got_quit
#if defined(FEAT_EVAL)
		|| aborting()
#endif
		); ++lnum)
    {
	nmatch = vim_regexec_multi(regmatch, NULL, curbuf,
		lnum, (colnr_T)0, NULL);
	...
	if (nmatch)
	{
		...
		if (flags->do_ask)
		{
			...
			if (exmode_active)
			{
			    ...
			    curwin->w_cursor.col = regmatch->startpos[0].col;
			    if (flags->do_number || curwin->w_p_nu)
			    {
